...
static bool _bc_sys_i2c_tag_write(bc_tag_transfer_t *transfer, bool *communication_fault)
{
	*communication_fault = true;
	...
	if (HAL_I2C_Mem_Write(&hi2c2, transfer->device_address << 1, transfer->address, transfer->address_16_bit ? I2C_MEMADD_SIZE_16BIT : I2C_MEMADD_SIZE_8BIT, transfer->buffer, transfer->length, 0xFFFFFFFF) != HAL_OK)
	{
		return false;
	}
...
static bool _bc_sys_i2c_tag_read(bc_tag_transfer_t *transfer, bool *communication_fault)
{
	*communication_fault = true;
	...
	if (HAL_I2C_Mem_Read(&hi2c2, transfer->device_address << 1, transfer->address, transfer->address_16_bit ? I2C_MEMADD_SIZE_16BIT : I2C_MEMADD_SIZE_8BIT, transfer->buffer, transfer->length, 0xFFFFFFFF) != HAL_OK)
	{
		return false;
	}
